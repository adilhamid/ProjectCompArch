...
// MDDP Implementation
class MDPPReplPolicy : public ReplPolicy {
protected:
...
public:
	MDPPReplPolicy(uint32_t _numLines, int pNumCands) :
	...
	// Table for Lookup of oldIndex to newIndex
	uint32_t lookUpTable(uint32_t oldIndex) {
	...
	// Table for Lookup of oldIndex to newIndex
	uint32_t acessUpdateTable(uint32_t oldIndex) {
		uint32_t newIndex = oldIndex;
		switch (oldIndex) {
		case 7:
		case 6:
	...
	void placementUpdate(int start, int end) {
		for (int i = 0; i < end - start; i++) {
			int temp = candArray[start + i];
			candArray[start + i] = candArray[start + i + 1];
			candArray[start + i + 1] = temp;
	...
	void update(uint32_t id, const MemReq* req) {
		...
		if (array[blockId*numCands] == 9999999) { //case 1 : when the block is replaced; MISS
			array[id] = id;
			candArray[0] = id;
			int endLocation = numCands / 4 - 1;
			placementUpdate(0, endLocation);
		}

		else {	//case 2 : when the block is updated : HIT
			uint32_t updatedId = getUpdatedId(id);
			uint32_t newIndex = lookUpTable(updatedId);
			if (updatedId != newIndex) {
				accessUpdate(acessUpdateTable(updatedId));
			}
		}
